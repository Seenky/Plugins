		...
		for (const FZoneLaneData& ZoneLaneData : ZoneGraphStorage.Lanes)
		{
			...
			if (ZoneGraphTagFilter.Pass(ZoneLaneData.Tags))
			{
				for (int32 PointIndex = ZoneLaneData.PointsBegin; PointIndex < ZoneLaneData.PointsEnd; PointIndex++)
				{
					...
					const bool bHit = SnapPointToGround(World, nullptr, LanePoint, LaneUpVector, LaneTangentVector);
					if (!bHit)
					{
						++MissCount;
					}
...
void AMassTrafficZoneGraphDataModifier::SnapZoneShapesToGround(TArray<UZoneShapeComponent*> Components)
{
	#if WITH_EDITOR
	...
	// Snap each Zone Graph point to the ground.
	{
		FScopedSlowTask SlowTask(Components.Num(), NSLOCTEXT("MassTraffic", "SnapZoneGraphDataToGround",
			"Snapping Zone Graph lane points to geometry..."));
		SlowTask.MakeDialog(true);
		...
		for (const auto Component : Components)
		{
			...
			for (auto& Point : Component->GetMutablePoints())
			{
				FVector ZeroPoint = FVector::ZeroVector;
				const bool bHit = SnapPointToGround(World,Component, Point.Position, ZeroPoint, ZeroPoint);
				if (!bHit)
				{
					++MissCount;
				}
...
bool AMassTrafficZoneGraphDataModifier::SnapPointToGround(UWorld* World, USceneComponent* ZoneComponent, FVector& Point, FVector& UpVector, FVector& TangentVector)
{
	FVector ComponentLocation;
	
	if (ZoneComponent)
	{
		ComponentLocation = ZoneComponent->GetComponentLocation();
	}
	else
	{
		ComponentLocation = FVector::ZeroVector;
	}
	const FVector FixedLocation = ComponentLocation + Point;
	const FVector TraceStart(FixedLocation.X, FixedLocation.Y, FixedLocation.Z + TraceStartZOffset);
	const FVector TraceEnd(FixedLocation.X, FixedLocation.Y, FixedLocation.Z + TraceEndZOffset);
	...
	if (bHit)
	{
		const FVector PointOrig = FixedLocation;
		
		if (bSnapPointZ)
		{
			Point.Z = TraceHitResult.ImpactPoint.Z - ComponentLocation.Z + /*optional*/TraceFinalZOffset;
		}
		...
		if (bTraceDebugDrawHits && IsPointNearActorLocation(Point))
		{
			...
			DrawDebugPoint(World, TraceHitResult.ImpactPoint, 40.0f, FColor::Red, false, DebugLifetime, 0.0f);
		}
