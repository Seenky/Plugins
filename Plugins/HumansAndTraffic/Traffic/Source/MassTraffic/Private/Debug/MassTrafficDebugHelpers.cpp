...
void DrawDebugShouldStop(float DebugDrawSize, const FColor DebugDrawColor, const FString DebugText, bool bVisLog, const UObject* VisLogOwner, const FTransform* VisLogTransform, const float SpaceTakenByVehicleOnLane)
{
#if ENABLE_DRAW_DEBUG || ENABLE_VISUAL_LOG 
	...
	if (GMassTrafficDebugShouldStop && FVector::Distance(GetPlayerViewLocation(VisLogOwner->GetWorld()), VisLogTransform->GetLocation()) <= ViewerDistance)
	{
		...
		DrawDebugBox(VisLogOwner->GetWorld(), VisLogTransform->GetLocation(), FVector(SpaceTakenByVehicleOnLane, SpaceTakenByVehicleOnLane, SpaceTakenByVehicleOnLane), FColor::Red, false, 0.0f);
	}
#endif
...
void DrawDebugEmergencyStopLanes(const UWorld* InWorld, FVector StartLocation, FVector EndLocation, float ArrowSize,
	FColor Color)
{
	if (GMassTrafficDebugEmergencyStop < 1) return;

	DrawDebugDirectionalArrow(InWorld, StartLocation, EndLocation, ArrowSize, Color, false, 0);
	DrawDebugBox(InWorld, EndLocation, FVector(50, 50, 50), FColor::Green, false, 0);
}

void DrawDebugEmergencyStopBox(const UWorld* InWorld, FTransform InTransform, FBox InBox, FColor Color)
{
	if (GMassTrafficDebugEmergencyStop < 1) return;
	
	DrawDebugBox(InWorld, InBox.GetCenter(), InBox.GetExtent(), Color, false, 0);
}
