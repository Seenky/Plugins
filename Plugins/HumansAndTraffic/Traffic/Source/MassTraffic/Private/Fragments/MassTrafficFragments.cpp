...
void FMassTrafficIntersectionFragment::ApplyLanesActionToSelectedPeriod(
...
void FMassTrafficIntersectionFragment::ApplyLanesActionToOpenedPeriods(
	const EMassTrafficPeriodLanesAction VehicleLanesAction,
	const EMassTrafficPeriodLanesAction PedestrianLanesAction,
	UMassCrowdSubsystem* MassCrowdSubsystem,
	const bool bForce) 
{
	for (const auto CurrentPeriod : OpenedPeriods)
	{
		if (!LastPeriodTrafficAction.Find(CurrentPeriod)) return;
		if (!LastPeriodPedestrianAction.Find(CurrentPeriod)) return;

		EMassTrafficPeriodLanesAction* LastActionTraffic = LastPeriodTrafficAction.Find(CurrentPeriod);
		EMassTrafficPeriodLanesAction* LastActionPedestrian = LastPeriodPedestrianAction.Find(CurrentPeriod);
		
		if ((VehicleLanesAction != *LastActionTraffic || bForce) &&
			VehicleLanesAction != EMassTrafficPeriodLanesAction::None)
		{
			for (int32 I = 0; I < CurrentPeriod.NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane); I++)
			{
			...
			*LastActionTraffic = VehicleLanesAction;
		}
	
		if ((PedestrianLanesAction != *LastActionPedestrian || bForce) &&
			PedestrianLanesAction != EMassTrafficPeriodLanesAction::None)
		{
			// Open or close all this period's pedestrian lanes.
			// NOTE - These is no soft-close for these lanes.
			...
			*LastActionPedestrian = PedestrianLanesAction;
		}
	}
}
...
void FMassTrafficIntersectionFragment::UpdateTrafficLightsForSelectedPeriods(TArray<FMassTrafficPeriod, TInlineAllocator<MASSTRAFFIC_NUM_INLINE_PERIODS>> SelectPeriods)
{
	if (!bHasTrafficLights)
	{
		return;	
	...
	if (OpenedPeriods.IsEmpty()) return;
	
	for (auto Period : SelectPeriods)
	{
		for (int8 I = 0; I < TrafficLights.Num(); I++)
		{
			const FMassTrafficLightControl* CurrentTrafficLightControl = Period.GetTrafficLightControl(I);		
			if (!CurrentTrafficLightControl)
			{
				continue;
			}
			...
			{
				const bool bIsCurrentPeriodAboutToEnd =
					(Period.Duration < 2.0f * MassTrafficSettings->StandardTrafficPrepareToStopSeconds ? 
						PeriodTimeRemaining < Period.Duration / 2.0f :
						PeriodTimeRemaining < MassTrafficSettings->StandardTrafficPrepareToStopSeconds);
