...
#include "Representation/MassTrafficTrailerRepresentationActorManagement.h"
	...
	, FConstStructView SpawnRequest, TSharedRef<FMassEntityManager> EntityManager) const
{
	...
	if (AWheeledVehiclePawn* TrailerPawn = Cast<AWheeledVehiclePawn>(MassActorSpawnRequest.SpawnedActor))
	{
		...
		if (VehicleConstraintFragment.Vehicle.IsSet())
		{
			...
			PackedCustomData = FMassTrafficVehicleInstanceCustomData::MakeTrafficVehicleCustomData(VehicleLightsFragment, TrailerRandomFractionFragment, false, FLinearColor::Transparent, FVector::ZeroVector, true);
