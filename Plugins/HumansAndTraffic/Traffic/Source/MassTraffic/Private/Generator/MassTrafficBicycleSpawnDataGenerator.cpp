...
#include "Generator/MassTrafficBicycleSpawnDataGenerator.h"

#include "MassCommonUtils.h"
#include "Processor/MassTrafficChooseNextLaneProcessor.h"
#include "Processor/MassTrafficFindNextVehicleProcessor.h"
#include "Processor/MassTrafficInitInterpolationProcessor.h"
#include "Processor/MassTrafficInitTrafficVehicleSpeedProcessor.h"
#include "Processor/MassTrafficInitTrafficVehiclesProcessor.h"
#include "Processor/MassTrafficUpdateDistanceToNearestObstacleProcessor.h"
#include "Processor/MassTrafficUpdateVelocityProcessor.h"
#include "Static/MassTrafficFieldOperations.h"
#include "Static/MassTrafficLaneChange.h"
#include "Subsystems/MassTrafficSubsystem.h"
#include "MassEntityConfigAsset.h"

void UMassTrafficBicycleSpawnDataGenerator::Generate(
	UObject& QueryOwner,
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(TEXT("MassTrafficBicycleSpawnDataGenerator"))
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, FString::Printf(TEXT("Spacing: %f"), DefaultSpacing.Space));
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, FString::Printf(TEXT("Entity types: %i"), EntityTypes.Num()));
	for (const FMassSpawnedEntityType& EntityType : EntityTypes)
	{
	...
	for (const FMassTrafficZoneGraphData& TrafficZoneGraphData : MassTrafficSubsystem->GetTrafficZoneGraphData())
	{
		...
		const bool bFoundPoints = FindNonOverlappingLanePoints(
			*ZoneGraphStorage,
			GetDefault<UMassTrafficSettings>()->BicycleLaneFilter, 
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, FString::Printf(TEXT("Results num: %i"), Results.Num()));
	...
	for (int32 SpacingIndex = 0; SpacingIndex < SpawnPointsPerSpacing.Num(); ++SpacingIndex)
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, FString::Printf(TEXT("Available spawn points num: %i"), AvailableSpawnPointsPerSpacing.Num()));
	}
	for (int32 ResultIndex = 0; ResultIndex < Results.Num(); ++ResultIndex)
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, FString::Printf(TEXT("Spacing index: %i"), SpacingIndex));
...
bool UMassTrafficBicycleSpawnDataGenerator::FindNonOverlappingLanePoints(
