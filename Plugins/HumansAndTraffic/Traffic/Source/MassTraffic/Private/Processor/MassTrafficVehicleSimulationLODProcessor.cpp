...
#include "Processor/MassTrafficVehicleSimulationLODProcessor.h"
...
void UMassTrafficVehicleSimulationLODProcessor::ConfigureQueries()
{
	...
	EntityQueryVariableTick.AddRequirement<FMassRepresentationLODFragment>(EMassFragmentAccess::ReadOnly);
	EntityQueryVariableTick.AddRequirement<FMassSimulationVariableTickFragment>(EMassFragmentAccess::ReadWrite);
	EntityQueryVariableTick.AddRequirement<FMassTrafficSimulationLODFragment>(EMassFragmentAccess::ReadWrite);
	...
	EntityQueryLODChange.AddRequirement<FMassRepresentationFragment>(EMassFragmentAccess::ReadOnly);
...
void UMassTrafficVehicleSimulationLODProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
...

	{
		...
		EntityQueryLODChange.ForEachEntityChunk(EntityManager, Context, [this](FMassExecutionContext& QueryContext)
		{
			...
			const TConstArrayView<FMassRepresentationLODFragment> SimulationLODFragments = QueryContext.GetFragmentView<FMassRepresentationLODFragment>();
			...
			{
				const FMassRepresentationFragment& RepresentationFragment = RepresentationFragments[EntityIdx];
				const FMassRepresentationLODFragment& SimulationLODFragment = SimulationLODFragments[EntityIdx];
...
#if WITH_MASSTRAFFIC_DEBUG
	{
		...
		{
			const int32 NumEntities = QueryContext.GetNumEntities();
			const TArrayView<FMassRepresentationLODFragment> SimulationLODFragments = QueryContext.GetMutableFragmentView<FMassRepresentationLODFragment>();

			for (int EntityIdx = 0; EntityIdx < NumEntities; EntityIdx++)
			{
				FMassRepresentationLODFragment& EntityLOD = SimulationLODFragments[EntityIdx];
