...
void UMassTrafficInterpolationProcessor::ConfigureQueries()
{
	...
	// Custom Requirement
	EntityQueryNonOffLOD_Conditional.AddRequirement<FMassTrafficMovementInterpSpeed>(EMassFragmentAccess::ReadWrite);
	EntityQueryOffLOD_Conditional.AddRequirement<FMassTrafficMovementInterpSpeed>(EMassFragmentAccess::ReadWrite);
}
...
void UMassTrafficInterpolationProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	EntityQueryNonOffLOD_Conditional.ForEachEntityChunk(EntityManager, Context, [&, World = EntityManager.GetWorld()](FMassExecutionContext& QueryContext)
	{
		...
		const TArrayView<FMassTrafficMovementInterpSpeed> InterpSpeedFragments = Context.GetMutableFragmentView<FMassTrafficMovementInterpSpeed>();
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			FMassTrafficMovementInterpSpeed& InterpSpeedFragment = InterpSpeedFragments[Index];
