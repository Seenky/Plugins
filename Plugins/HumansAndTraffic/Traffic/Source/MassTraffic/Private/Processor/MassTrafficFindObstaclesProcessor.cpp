...
#include "Processor/MassTrafficFindObstaclesProcessor.h"
...

	{
		...
		ObstacleEntityQuery.ForEachEntityChunk(EntityManager, Context, [&](FMassExecutionContext& QueryContext)
		{
			...
			for (int32 Index = 0; Index < NumEntities; ++Index)
			{
				...
				TArray<FZoneGraphLaneHandle> NearbyBicycleLanes;
				ZoneGraphSubsystem.FindOverlappingLanes(SearchBox, GetDefault<UMassTrafficSettings>()->TrafficLaneFilter, NearbyLanes);
				ZoneGraphSubsystem.FindOverlappingLanes(SearchBox, GetDefault<UMassTrafficSettings>()->BicycleLaneFilter, NearbyBicycleLanes);
				NearbyLanes.Append(NearbyBicycleLanes);
