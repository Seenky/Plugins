...
#include "Traits/MassTrafficTrailerSimulationTrait.h"
...
void UMassTrafficVehiclePhysicsProcessor::ConfigureQueries()
{
	...
	SimplePhysicsVehiclesQuery.AddRequirement<FMassRepresentationFragment>(EMassFragmentAccess::ReadWrite);
	...
	// Advance agents
	{
		...
		SimplePhysicsVehiclesQuery.ForEachEntityChunk(EntityManager, Context, [&](FMassExecutionContext& QueryContext)
		{
			...
			for (int32 Index = 0; Index < NumEntities; ++Index)
			{
				...
				FMassRepresentationFragment& RepresentationFragment = VisualizationFragments[Index];
				...
				if (RepresentationFragment.CurrentRepresentation == EMassRepresentationType::StaticMeshInstance || RepresentationFragment.CurrentRepresentation == EMassRepresentationType::None) continue;
				...
				if (VehicleControlFragment.bUseFixedTransform &&
					FVector::Distance(RawLaneLocationTransform.GetLocation(), VehicleControlFragment.FixedVehicleTransform.GetLocation()) < MassTrafficSettings->DeviantLocationFixMax)
					RawLaneLocationTransform.AddToTranslation(VehicleControlFragment.FixedVehicleTransform.GetTranslation() - RawLaneLocationTransform.GetTranslation());
				if (VehicleControlFragment.bUseFixedTransform)
					RawLaneLocationTransform.SetRotation(VehicleControlFragment.FixedVehicleTransform.GetRotation());
...
)
{
	...
	
	{
		...
		SimplePhysicsVehicleFragment.WorldRotationOfMass = NewVehicleWorldRotationOfMass;
	...
	SimplePhysicsVehicleFragment.AngularVelocity = AngularVelocityFragment.AngularVelocity;
	SimplePhysicsVehicleFragment.LinearAcceleration = TotalForce / SimplePhysicsVehicleFragment.VehicleSim.Setup().Mass;

	FVector TotalSuspensionForce(ForceInitToZero);
	for (const auto& SuspensionSim : SimplePhysicsVehicleFragment.VehicleSim.SuspensionSims)
	{
		TotalSuspensionForce += VehicleWorldUpAxis * SuspensionSim.GetSuspensionForce();
	}
	
	SimplePhysicsVehicleFragment.SuspensionForces = TotalSuspensionForce;
	SimplePhysicsVehicleFragment.SuspensionTorque = TotalTorque;

	// NaN check
	if (!(TransformFragment.GetTransform().IsValid()))
	{
