...
#include "Processor/MassTrafficUpdateIntersectionsProcessor.h"
#include "MassTraffic.h"
#include "Fragments/MassTrafficFragments.h"
#include "Debug/MassTrafficDebugHelpers.h"
...
namespace 
{
	...
	FORCEINLINE bool ArePedestriansClearOfIntersection(FMassTrafficIntersectionFragment& IntersectionFragment, const FZoneGraphStorage& ZoneGraphStorage, const UMassCrowdSubsystem* MassCrowdSubsystem)
	{
	    ...
	    // Check OpenedPeriods
	    for (const FMassTrafficPeriod& OpenedPeriod : IntersectionFragment.OpenedPeriods)
	    {
	        for (const int32 CrosswalkLaneIndex : OpenedPeriod.CrosswalkLanes)
	        {
	            const FZoneGraphLaneHandle LaneHandle(CrosswalkLaneIndex, ZoneGraphStorage.DataHandle);
	            if (!LaneHandle.IsValid())
	            {
	...
	FORCEINLINE bool AreVehiclesClearOfIntersection(FMassTrafficIntersectionFragment& IntersectionFragment, const EMassTrafficIntersectionVehicleLaneType ClearTest, const bool bIncludeReservedVehicles = true)
	{
		...
		if (!IntersectionFragment.OpenedPeriods.IsEmpty())
		{
			for (const auto Period : IntersectionFragment.OpenedPeriods)
			{
				for (int32 I = 0; I < Period.NumVehicleLanes(ClearTest); I++)
				{
					const FZoneGraphTrafficLaneData* VehicleLane = Period.GetVehicleLane(I, ClearTest);
					
					if (!VehicleLane->LaneHandle.IsValid()) continue;
	...
	FORCEINLINE bool AreVehiclesWaitingForIntersection(FMassTrafficIntersectionFragment& IntersectionFragment)
	{
		...
		if (!IntersectionFragment.OpenedPeriods.IsEmpty())
		{
			for (const auto Period : IntersectionFragment.OpenedPeriods)
			{
				for (const FZoneGraphTrafficLaneData* VehicleLane : Period.VehicleLanes)
				{
					if (VehicleLane->bIsVehicleReadyToUseLane)
					{
						return true;
					}
				}
	...
	FORCEINLINE int32 NumVehiclesWaitingForIntersection(FMassTrafficIntersectionFragment& IntersectionFragment)
	{
		...
		if (!IntersectionFragment.OpenedPeriods.IsEmpty())
		{
			for (const auto Period : IntersectionFragment.OpenedPeriods)
			{
				for (const FZoneGraphTrafficLaneData* TrafficLaneData : Period.VehicleLanes)
				{
					if (TrafficLaneData->bIsVehicleReadyToUseLane) // (See all READYLANE.)
					{
						++Count;
	...
	FORCEINLINE int32 NumPedestriansWaitingForIntersection(FMassTrafficIntersectionFragment& IntersectionFragment, const FZoneGraphStorage& ZoneGraphStorage, const UMassCrowdSubsystem* MassCrowdSubsystem, const UWorld* Wold = nullptr/*..for debugging*/)
	{
	    ...
	    auto ProcessWaitLanes = [&](const TArray<int32>& CrosswalkWaitingLanes)
	    {
	        for (const int32 CrosswalkWaitingLaneIndex : CrosswalkWaitingLanes)
	        {
	            ...
	            if (!CrowdWaitAreaData) continue;

	            // Check duplicates
	            bool bAlreadyCounted = false;
	            for (const FCrowdWaitAreaData* CountedData : CountedCrowdWaitAreaDataArray)
	            {
	                if (CountedData == CrowdWaitAreaData)
	                {
	                    bAlreadyCounted = true;
	                    break;
	                }
	                if (!CountedData) break;
	            }
	            if (bAlreadyCounted) continue;

	            // Add to counted array
	            int32 FreeSlotIdx = 0;
	            while (FreeSlotIdx < MAX_COUNTED_CROWD_WAIT_AREA_ARRAY && CountedCrowdWaitAreaDataArray[FreeSlotIdx]) ++FreeSlotIdx;
	            if (FreeSlotIdx < MAX_COUNTED_CROWD_WAIT_AREA_ARRAY)
	            {
	                CountedCrowdWaitAreaDataArray[FreeSlotIdx] = CrowdWaitAreaData;
	    ...
	    // Check CurrentPeriod
	    const FMassTrafficPeriod& CurrentPeriod = IntersectionFragment.GetCurrentPeriod();
	    ProcessWaitLanes(CurrentPeriod.CrosswalkWaitingLanes);

	    // Check OpenedPeriods
	    for (const FMassTrafficPeriod& OpenedPeriod : IntersectionFragment.OpenedPeriods)
	    {
	        ProcessWaitLanes(OpenedPeriod.CrosswalkWaitingLanes);
	    }
	...
	FORCEINLINE int32 NumPedestriansCrossing(FMassTrafficIntersectionFragment& IntersectionFragment, const FZoneGraphStorage& ZoneGraphStorage, const UMassCrowdSubsystem* MassCrowdSubsystem)
	{
		...
		auto ProcessCrosswalkLanes = [&](const TArray<int32, TInlineAllocator<MASSTRAFFIC_NUM_INLINE_PERIOD_PEDESTRIAN_CROSSWALK_LANES>>& CrosswalkLanes)
		{
		...
		// Check CurrentPeriod
		const FMassTrafficPeriod& CurrentPeriod = IntersectionFragment.GetCurrentPeriod();
		ProcessCrosswalkLanes(CurrentPeriod.CrosswalkLanes);

		// Check OpenedPeriods
		for (const FMassTrafficPeriod& OpenedPeriod : IntersectionFragment.OpenedPeriods)
		{
			ProcessCrosswalkLanes(OpenedPeriod.CrosswalkLanes);
		}
	...
	void DrawDebugVehicleLaneArrows(const UWorld* World, const FZoneGraphStorage& ZoneGraphStorage, FMassTrafficIntersectionFragment& IntersectionFragment, const float Lifetime)
	{
		...
		for (const FZoneGraphTrafficLaneData* VehicleLane : CurrentPeriod.VehicleLanes)
		{
			const float Thickness = (VehicleLane->bIsVehicleReadyToUseLane ? 20.0f : 5.0f);
		...
		for (const FMassTrafficPeriod& OpenedPeriod : IntersectionFragment.OpenedPeriods)
		{
			for (const FZoneGraphTrafficLaneData* VehicleLane : OpenedPeriod.VehicleLanes)
			{
				if (!VehicleLane->LaneHandle.IsValid()) continue;
	...
	{
		auto DrawLanes = [&](const FMassTrafficPeriod& Period)
		{
			// Crosswalk Lanes
			for (const int32 CrosswalkLaneIndex : Period.CrosswalkLanes)
			{
				const FZoneGraphLaneHandle LaneHandle(CrosswalkLaneIndex, ZoneGraphStorage.DataHandle);
			...
			// Crosswalk Waiting Lanes
			for (const int32 CrosswalkWaitingLaneIndex : Period.CrosswalkWaitingLanes)
			{
				const FZoneGraphLaneHandle LaneHandle(CrosswalkWaitingLaneIndex, ZoneGraphStorage.DataHandle);
		...
		const FMassTrafficPeriod& CurrentPeriod = IntersectionFragment.GetCurrentPeriod();
		DrawLanes(CurrentPeriod);
		
		for (const FMassTrafficPeriod& OpenedPeriod : IntersectionFragment.OpenedPeriods)
		{
			DrawLanes(OpenedPeriod);
		}	
	}
...
void UMassTrafficUpdateIntersectionsProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(EntityManager, Context, [&, World](FMassExecutionContext& QueryContext)
	{
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			if (GMassTrafficDebugIntersections)
			{
				...
				if (IntersectionFragment.ZoneGraphDataHandle.IsValid())
				{
			...
			bool bPeriodHasAnyOpenVehicleLanes = false;
			{
				...
				if (!IntersectionFragment.OpenedPeriods.IsEmpty())
				{
					...
					{
						for (int32 VehicleLaneLaneIndex = 0;
						VehicleLaneLaneIndex < Period.NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane);
						VehicleLaneLaneIndex++)
						{
							const FZoneGraphTrafficLaneData* VehicleLane = Period.GetVehicleLane(VehicleLaneLaneIndex, EMassTrafficIntersectionVehicleLaneType::VehicleLane);
							if (VehicleLane->bIsOpen)
							{
								bPeriodHasAnyOpenVehicleLanes = true;
								break;
			...
			bool bPeriodHasAnyOpenCrosswalkLanes = false;
			{
				...
				for (auto Period : IntersectionFragment.OpenedPeriods)
				{
					for (const int32 CrosswalkLaneIndex : Period.CrosswalkLanes)
					{
						const FZoneGraphLaneHandle LaneHandle(CrosswalkLaneIndex, IntersectionFragment.ZoneGraphDataHandle);
	...
	if (bDoDrawDebug)
	{
		DrawDebugString(
			GetWorld(),
			TransformFragment.GetTransform().GetLocation() + FVector(0, 0, 600),
			FString::Printf(TEXT("NUM PERIODS : %i"), IntersectionFragment.Periods.Num()),
			nullptr, FColor::Purple,
			0.f);
	}
#endif
			...
			if (IntersectionFragment.PeriodTimeRemaining > 0.0f)
			{
				...
				IntersectionFragment.UpdateTrafficLightsForSelectedPeriods(IntersectionFragment.OpenedPeriods);
				...
				IntersectionFragment.PeriodTimeRemaining > 0.0f/*optimization*/)
			{
				...
				IntersectionFragment.ApplyLanesActionToOpenedPeriods(
								EMassTrafficPeriodLanesAction::SoftPrepareToClose,
								EMassTrafficPeriodLanesAction::None,
								&MassCrowdSubsystem, false);
				...
				if (!IntersectionFragment.OpenedPeriods.IsEmpty())
				{
					for (auto Period : IntersectionFragment.OpenedPeriods)
					{
						for (int32 LaneIndex = 0; LaneIndex < Period.NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane_ClosedInNextPeriod); LaneIndex++)
						{
							FZoneGraphTrafficLaneData* OpenVehicleLane = Period.GetVehicleLane(LaneIndex, EMassTrafficIntersectionVehicleLaneType::VehicleLane_ClosedInNextPeriod);
			...
			if (IntersectionFragment.PeriodTimeRemaining <= 0.0f && PeriodTimeRemaining_BeforeUpdate <= 0.0f)
			{
				...
				{
					EMassTrafficPeriodLanesAction VehicleLanesAction = EMassTrafficPeriodLanesAction::SoftClose;
					EMassTrafficPeriodLanesAction PedestrianLanesAction = EMassTrafficPeriodLanesAction::HardClose;
					...
					// НОВАЯ ЛОГИКА: Предотвращаем одновременное открытие
					if (AreVehiclesWaitingForIntersection(IntersectionFragment))
					{
						VehicleLanesAction = EMassTrafficPeriodLanesAction::Open;
						// Гарантируем закрытие пешеходных переходов при открытии для машин
						PedestrianLanesAction = EMassTrafficPeriodLanesAction::HardClose;
					}
					else if (bCanOpenPedestrianLanesByProbability &&
							 NumPedestriansWaitingForIntersection(IntersectionFragment, *ZoneGraphStorage, &MassCrowdSubsystem) >= MinPedestrians &&
							 !IsStoppedVehicleBlockingCrosswalk(IntersectionFragment, true))
					{
						PedestrianLanesAction = EMassTrafficPeriodLanesAction::Open;
						// Гарантируем закрытие машинных полос при открытии для пешеходов
						VehicleLanesAction = EMassTrafficPeriodLanesAction::HardClose;
}
					...
					if (IntersectionFragment.LastVehicleLanesActionAppliedToCurrentPeriod == EMassTrafficPeriodLanesAction::Open)
					{
						IntersectionFragment.OpenedPeriods.Empty();
						for (int32 PeriodIndex = 0; PeriodIndex < IntersectionFragment.Periods.Num(); PeriodIndex++)
						{
							if (PeriodIndex == IntersectionFragment.CurrentPeriodIndex) 
								continue;

							FMassTrafficPeriod& OtherPeriod = IntersectionFragment.Periods[PeriodIndex];
							bool bShouldClose = false;

							if (OtherPeriod.VehicleLanes.IsEmpty() &&
								(!OtherPeriod.CrosswalkLanes.IsEmpty() || !OtherPeriod.CrosswalkWaitingLanes.IsEmpty()))
							{
								continue;
							}
								
							for (int32 CurrentVehicleLaneIndex = 0; 
							CurrentVehicleLaneIndex < IntersectionFragment.Periods[IntersectionFragment.CurrentPeriodIndex].NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane); 
							CurrentVehicleLaneIndex++)
							{
								if (bShouldClose) break;
							
								FZoneGraphTrafficLaneData* CurrentLane = IntersectionFragment.Periods[IntersectionFragment.CurrentPeriodIndex].GetVehicleLane(
								CurrentVehicleLaneIndex, 
								EMassTrafficIntersectionVehicleLaneType::VehicleLane
								);
								if (!CurrentLane->LaneHandle.IsValid()) 
									continue;

								for (int32 OtherVehicleLaneIndex = 0; 
								OtherVehicleLaneIndex < OtherPeriod.NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane); 
								OtherVehicleLaneIndex++)
								{
									FZoneGraphTrafficLaneData* OtherLane = OtherPeriod.GetVehicleLane(
									OtherVehicleLaneIndex, 
									EMassTrafficIntersectionVehicleLaneType::VehicleLane
									);
									if (!OtherLane->LaneHandle.IsValid() || CurrentLane->LaneHandle == OtherLane->LaneHandle)
										continue;

									if (DoZonesIntersect(GetWorld(), *ZoneGraphStorage, CurrentLane, OtherLane))
									{
										bShouldClose = true;
										break;
									}
							...
							if (!bShouldClose)
							{
								IntersectionFragment.OpenedPeriods.AddUnique(OtherPeriod);
							}
							else
							{
								IntersectionFragment.OpenedPeriods.Remove(OtherPeriod);
							}
						}

						if (IntersectionFragment.OpenedPeriods.Num() > 0)
					    {
					        TArray<FMassTrafficPeriod, TInlineAllocator<14>> ShuffledPeriods = IntersectionFragment.OpenedPeriods;
					        ShuffledPeriods.Sort([](const FMassTrafficPeriod& A, const FMassTrafficPeriod& B) {
					            return FMath::RandBool();
					        });

					        TArray<FMassTrafficPeriod> FilteredPeriods;
					        for (const FMassTrafficPeriod& Period : ShuffledPeriods)
					        {
					            bool bIntersects = false;
					            for (const FMassTrafficPeriod& SelectedPeriod : FilteredPeriods)
					            {
					                for (int32 LaneIndex = 0; LaneIndex < Period.NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane); LaneIndex++)
					                {
					                    FZoneGraphTrafficLaneData* Lane = Period.GetVehicleLane(LaneIndex, EMassTrafficIntersectionVehicleLaneType::VehicleLane);
					                    if (!Lane || !Lane->LaneHandle.IsValid()) continue;

					                    for (int32 SelectedLaneIndex = 0; SelectedLaneIndex < SelectedPeriod.NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane); SelectedLaneIndex++)
					                    {
					                        FZoneGraphTrafficLaneData* SelectedLane = SelectedPeriod.GetVehicleLane(SelectedLaneIndex, EMassTrafficIntersectionVehicleLaneType::VehicleLane);
					                        if (!SelectedLane || !SelectedLane->LaneHandle.IsValid()) continue;

					                        if (DoZonesIntersect(GetWorld(), *ZoneGraphStorage, Lane, SelectedLane))
					                        {
					                            bIntersects = true;
					                            break;
					                        }
					        ...
					        IntersectionFragment.OpenedPeriods = FilteredPeriods;
							
					        if (IntersectionFragment.OpenedPeriods.Num() == 0)
					        {
					            int32 RandomIndex = FMath::RandRange(0, ShuffledPeriods.Num() - 1);
				        		FMassTrafficPeriod SelectedPeriod = ShuffledPeriods[RandomIndex];
				        		for (auto Period : ShuffledPeriods)
				        		{
				        			bool bReadyToUseLane = false;
				        			for (int32 LaneIndex = 0; LaneIndex < Period.NumVehicleLanes(EMassTrafficIntersectionVehicleLaneType::VehicleLane); LaneIndex++)
				        			{
				        				FZoneGraphTrafficLaneData* Lane = Period.GetVehicleLane(LaneIndex, EMassTrafficIntersectionVehicleLaneType::VehicleLane);
				        				if (Lane->bIsVehicleReadyToUseLane) bReadyToUseLane = true;
				        			}
									if (*IntersectionFragment.LastPeriodTrafficAction.Find(SelectedPeriod) != EMassTrafficPeriodLanesAction::Open)
									{
										SelectedPeriod = Period;
									}
				        			if (!bReadyToUseLane) ShuffledPeriods.Remove(Period);
				        		}
					            IntersectionFragment.OpenedPeriods.Add(SelectedPeriod);
					        }
					    }
					}
...
bool UMassTrafficUpdateIntersectionsProcessor::DoZonesIntersect(const UWorld* World,
	const FZoneGraphStorage& ZoneStorage, const FZoneGraphTrafficLaneData* ZoneA,
	const FZoneGraphTrafficLaneData* ZoneB)
{
	if (ZoneA->LaneHandle == ZoneB->LaneHandle) 
		return false;

	TArray<FVector> APoints;
	
	for (float StepLaneA = 0; StepLaneA < ZoneA->Length; StepLaneA += 10.0f)
	{
		FZoneGraphLaneLocation NewLocation;
		UE::ZoneGraph::Query::CalculateLocationAlongLane(ZoneStorage, ZoneA->LaneHandle.Index, StepLaneA, NewLocation);
		#if WITH_MASSTRAFFIC_DEBUG
		if (GMassTrafficDebugIntersections)
		{
			DrawDebugPoint(World, NewLocation.Position, 5.0f, FColor::Red, false, 3.0f);
		}
		#endif
		APoints.Add(NewLocation.Position);
	}
	
	TArray<FZoneGraphLaneHandle> OverlappingLanes;
	for (const auto PointA : APoints)
	{
		const FZoneGraphTagFilter EmptyFilter;
		const FBox BoundsA = FBox::BuildAABB(PointA, FVector(50, 50, 50));
		UE::ZoneGraph::Query::FindOverlappingLanes(ZoneStorage, BoundsA, EmptyFilter, OverlappingLanes);
		if (OverlappingLanes.Contains(ZoneB->LaneHandle))
		{
			#if WITH_MASSTRAFFIC_DEBUG
			if (GMassTrafficDebugIntersections)
			{
				DrawDebugBox(World, BoundsA.GetCenter(), BoundsA.GetExtent(), FColor::Blue, false, 3.0f);
			}
			#endif
			return true;
		}
