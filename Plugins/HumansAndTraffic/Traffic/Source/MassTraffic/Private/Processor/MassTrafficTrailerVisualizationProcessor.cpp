...
#include "Processor/MassTrafficTrailerVisualizationProcessor.h"
...
void UMassTrafficTrailerUpdateCustomVisualizationProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(EntityManager, Context, [this, &EntityManager](FMassExecutionContext& QueryContext)
	{
		...
		const TArrayView<FMassTrafficVehicleColorsFragment> VehicleColorsFragments = QueryContext.GetMutableFragmentView<FMassTrafficVehicleColorsFragment>();
		for (int32 EntityIndex = 0; EntityIndex < NumEntities; EntityIndex++)
		{
			...
			FMassTrafficVehicleColorsFragment& VehicleColorsFragment = VehicleColorsFragments[EntityIndex];
			...
			if (!(EntityManager.IsEntityValid(ConstrainedVehicleFragment.Vehicle)))
			{
				continue;
			}
			...
			const FMassTrafficPackedVehicleInstanceCustomData PackedCustomData =
				FMassTrafficVehicleInstanceCustomData::MakeTrafficVehicleCustomData(
					VehicleLightsFragment, RandomFractionFragment, false,
					VehicleColorsFragment.SelectedColor,
					TransformFragment.GetTransform().GetRotation().GetRightVector(), true);
			...
			switch (RepresentationFragment.CurrentRepresentation)
			{
				case EMassRepresentationType::LowResSpawnedActor:
				{
					if (AActor* Actor = ActorFragment.GetMutable())
					{
						...
						Actor->ForEachComponent<UPrimitiveComponent>(/*bIncludeFromChildActors*/true, [&PackedCustomData, DataFloats](UPrimitiveComponent* PrimitiveComponent)
						{
