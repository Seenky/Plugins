...
void DrawDebugShouldStop(float DebugDrawSize, const FColor DebugDrawColor, const FString DebugText, bool bVisLog = false, const UObject* VisLogOwner = nullptr, const FTransform* VisLogTransform = nullptr, const float SpaceTakenByVehicleOnLane = 0);
...
void VisLogMalformedNextLaneLinks(const FMassEntityManager& EntityManager, int32 LaneIndex, const FMassEntityHandle TailVehicle, const FMassEntityHandle UndiscoveredVehicle, int32 MarchEjectAt, const UObject* VisLogOwner);
	void DrawDebugEmergencyStopLanes(const UWorld* InWorld, FVector StartLocation, FVector EndLocation, float ArrowSize, FColor Color);
