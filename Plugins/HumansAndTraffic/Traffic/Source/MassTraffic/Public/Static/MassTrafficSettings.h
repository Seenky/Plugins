...
public:
	...
	/** Zone graph lane filter to identify lanes traffic bicycles can drive on. */
	UPROPERTY(EditDefaultsOnly, Config, Category = "Lanes")
	FZoneGraphTagFilter BicycleLaneFilter;
	...
	/**Zone graph lane filter to select parking lines*/
	UPROPERTY(EditDefaultsOnly, Config, Category = "Lanes")
	FZoneGraphTagFilter ParkingLaneFilter;
	...
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Gates")
	float MinDistanceToStopBeforeGates = 1000.f;
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Gates")
	float MaxDistanceToStopBeforeGates = 4000.f;
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Gates")
	float DistancePassAfterGates = 500.f;
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Gates")
	float GateNearSpeedFactor = 0.3f;
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Gates")
	float DistanceToGatesWork = 4000.0f;
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Gates")
	FName GateTag = "Gate";

	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Speed Bumps")
	float DistanceToSlowDown = 4000.f;
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Speed Bumps")
	float DistanceToPassSpeedBump = 500.f;
	UPROPERTY(EditDefaultsOnly, Config, Category = "Traffic Speed Bumps")
	float SlowDownScale = 0.3;

	UPROPERTY(EditAnywhere, Config, Category="Visualization")
	bool bEnableLocationFix = true;
	UPROPERTY(EditAnywhere, Config, Category="Visualization")
	float LocationFixLength = 2000.0f;
	UPROPERTY(EditAnywhere, Config, Category="Visualization", meta = (ClampMin = "0.1", ClampMax = "1.0"))
	float WheelRadiusPercent = 0.7f;
	UPROPERTY(EditAnywhere, Config, Category="Visualization")
	float RelativeErrorDistance = 10.0f;
	UPROPERTY(EditAnywhere, Config, Category="Visualization")
	float DeviantLocationFixMax = 4000.0f;
	UPROPERTY(EditAnywhere, Config, Category="Visualization")
	float MaxLowerDistance = 400.0f;
	UPROPERTY(EditAnywhere, Config, Category="Visualization")
	TEnumAsByte<ECollisionChannel> TransformFixCollisionChannel = ECC_Visibility;
	UPROPERTY(EditAnywhere, Config, Category="Visualization")
	TEnumAsByte<ECollisionChannel> TransformFixOccludCheckChannel = ECC_Visibility;

	UPROPERTY(EditAnywhere, Config, Category="Apperiance")
	TArray<FLinearColor> CarsColors;

	UPROPERTY(EditAnywhere, Category="Parking")
	float ParkLanesSearchRadius = 2000.0f;
