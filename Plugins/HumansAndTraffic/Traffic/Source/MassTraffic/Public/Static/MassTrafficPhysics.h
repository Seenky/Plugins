...
struct MASSTRAFFIC_API FMassTrafficVehiclePhysicsFragment : public FMassFragment
{
	...
	UPROPERTY(Transient)
	FVector SuspensionForces = FVector();

	UPROPERTY(Transient)
	FVector SuspensionTorque = FVector();
    
	UPROPERTY(Transient)
	FVector LinearAcceleration = FVector();

	UPROPERTY(Transient)
	FVector SuspensionOffset = FVector();

	UPROPERTY(Transient)
	FVector SuspensionVelocity = FVector();

	UPROPERTY(Transient)
	FQuat WorldRotationOfMass = FQuat();
};
