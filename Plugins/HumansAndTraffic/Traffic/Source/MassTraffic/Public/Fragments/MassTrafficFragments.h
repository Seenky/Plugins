...
#include "Data/MassTrafficDamage.h"
#include "Debug/MassTrafficDebugHelpers.h"
#include "Data/MassTrafficPIDController.h"
#include "Data/MassTrafficTypes.h"
#include "Static/MassTrafficUtils.h"
...
struct FMassTrafficPeriod
{
	...
	bool operator==(const FMassTrafficPeriod& Other) const
	{
		return PeriodGuid == Other.PeriodGuid;
	}
	...
	TArray<int32> CrosswalkWaitingLanes;
...
static uint32 GetTypeHash(const FMassTrafficPeriod& FloatAndID)
{
	return GetTypeHash(FloatAndID.PeriodGuid);
}
...
struct MASSTRAFFIC_API FMassTrafficIntersectionFragment : public FMassFragment
{
	...
	TArray<FMassTrafficPeriod, TInlineAllocator<MASSTRAFFIC_NUM_INLINE_PERIODS>> OpenedPeriods;
	TMap<FMassTrafficPeriod, EMassTrafficPeriodLanesAction> LastPeriodTrafficAction;
	TMap<FMassTrafficPeriod, EMassTrafficPeriodLanesAction> LastPeriodPedestrianAction;
	TMap<FZoneGraphTrafficLaneData*, FPoly> CachedLanePolys;
	...
	*/
	...
	FORCEINLINE FMassTrafficPeriod& AddPeriod(const float Duration)
	{
		...
		Period.PeriodGuid = FGuid::NewGuid();
		LastPeriodTrafficAction.Add(Period, EMassTrafficPeriodLanesAction::None);
		LastPeriodPedestrianAction.Add(Period, EMassTrafficPeriodLanesAction::None);
		return Period;
	}
	...
	const FMassTrafficPeriod& CurrentPeriod,
	const EMassTrafficPeriodLanesAction VehicleLanesAction,
	const EMassTrafficPeriodLanesAction PedestrianLanesAction,
	UMassCrowdSubsystem* MassCrowdSubsystem,
	const bool bForce);

	void ApplyLanesActionToOpenedPeriods(
	const EMassTrafficPeriodLanesAction VehicleLanesAction,
	const EMassTrafficPeriodLanesAction PedestrianLanesAction,
	UMassCrowdSubsystem* MassCrowdSubsystem,
	const bool bForce);
	...
	void UpdateTrafficLightsForSelectedPeriods(TArray<FMassTrafficPeriod, TInlineAllocator<MASSTRAFFIC_NUM_INLINE_PERIODS>> SelectPeriods);
...
struct MASSTRAFFIC_API FMassTrafficVehicleControlFragment : public FMassFragment
{
	...
	/*Fixed location of vehicle after trace*/
	FTransform FixedVehicleTransform = FTransform::Identity;
	bool bUseFixedTransform = false;
	FVector FixedFrontLocation= FVector::ZeroVector;
	FVector FixedRearLocation = FVector::ZeroVector;
	...
	FRotator AngleForward = FRotator::ZeroRotator;
	float ForWheelsRotateYawAngle = 0.0f;
	bool bShouldStop = false;
	bool bShouldSlowDown = false;
	float DistanceToSpeedBump = 0.0f;
	bool bPassedGate = false;
	bool bNeedStop = false;
	float StopInterpSpeed = 50.0f;
...
USTRUCT()
struct MASSTRAFFIC_API FMassTrafficMovementInterpSpeed : public FMassFragment
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere)
	float MovementInterpLocationSpeed = 0.f;
	UPROPERTY(EditAnywhere)
	float MovementInterpRotationSpeed = 1.f;
};

USTRUCT()
struct MASSTRAFFIC_API FMassTrafficGatesFragment : public FMassFragment
{
	GENERATED_BODY()
...
USTRUCT()
struct MASSTRAFFIC_API FMassTrafficEmergencyStopFragment : public FMassFragment
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere)
	float DistanceToStop = 200.f;
};

USTRUCT()
struct MASSTRAFFIC_API FMassTrafficVehicleColorsFragment : public FMassFragment
{
	GENERATED_BODY()

	FMassTrafficVehicleColorsFragment()
