...
#include "MassEntityZoneGraphSpawnPointsGeneratorAdvanced.h"
...
#include "SpawnerGeneratorSettings.h"
#include "Kismet/GameplayStatics.h"

void UMassEntityZoneGraphSpawnPointsGeneratorAdvanced::Generate(UObject& QueryOwner, TConstArrayView<FMassSpawnedEntityType> EntityTypes, int32 Count, FFinishedGeneratingSpawnDataSignature& FinishedGeneratingSpawnPointsDelegate) const
{
	if (Count <= 0)
	{
...
void UMassEntityZoneGraphSpawnPointsGeneratorAdvanced::GeneratePointsForZoneGraphData(const ::AZoneGraphData& ZoneGraphData, TArray<FVector>& Locations, const FRandomStream& RandomStream) const
{
	...
	const USpawnerGeneratorSettings* MassSettings = GetDefault<USpawnerGeneratorSettings>();
	...
	for (int32 LaneIndex = 0; LaneIndex < ZoneGraphStorage.Lanes.Num(); ++LaneIndex)
	{
		...
		if (TagFilter.Pass(Lane.Tags))
		{
			...
			while (Distance <= LaneLength && FVector::Distance(UGameplayStatics::GetPlayerPawn(GetWorld(), 0)->GetActorLocation(), LaneLocation.Position) < MassSettings->DistanceToPlayer)
			{
