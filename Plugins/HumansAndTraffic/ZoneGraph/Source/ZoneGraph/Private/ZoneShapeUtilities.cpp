									...
									FZoneGraphStorage& OutZoneStorage, TArray<FZoneShapeLaneInternalLink>& OutInternalLinks)
{
	...
	for (int32 PointIndex = Destinations.Num() - 1; PointIndex >= 0; PointIndex--)
	{
		...
		// Discard destination that would result in left or right or straight turns.
		if (EnumHasAnyFlags(Restrictions, EZoneShapeLaneConnectionRestrictions::NoLeftTurn | 
								 EZoneShapeLaneConnectionRestrictions::NoRightTurn |
								 EZoneShapeLaneConnectionRestrictions::NoStraight)) 
		{
			...
			const bool bRemoveStraight = EnumHasAnyFlags(Restrictions, EZoneShapeLaneConnectionRestrictions::NoStraight); 
			...
			const bool bIsRightTurn = FVector::DotProduct(SourceSide, DirToDest) < 0.0f;
			const bool bIsStraight = !bIsTurning; 

			if (bIsTurning)
			{
				const bool bSkip = (bIsLeftTurn && bRemoveLeft) || (bIsRightTurn && bRemoveRight);
				if (bSkip)
				{
					continue;
				}
			}
			else if (bIsStraight && bRemoveStraight) 
			{
				continue;
			}
		}
	...
	if (UE::ZoneGraph::Debug::bFillEmptyDestination)
	{
		...
		for (int32 DestSlotIdx = 0; DestSlotIdx < DestConnectionCount.Num(); DestSlotIdx++)
		{
			...
			const bool bNoStraight = EnumHasAnyFlags(DestSlot.Restrictions, EZoneShapeLaneConnectionRestrictions::NoStraight);
			if (bMergeLanesToOneDestinationLane || bOneLanePerEntry || bNoStraight)
			{
				continue;
			}
